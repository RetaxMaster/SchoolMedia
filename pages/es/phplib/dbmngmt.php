...
// include_once("".$eStoreDir."includes/configure.php"); Hata que se desbloquee el directorio de tienda, en ese momento se puede quitar el comentario a esta linea
	...
	//global DATABASE_PREFIX; // Se toma del archivo de inclusión sosemV10config.php
...
// Función para la base de datos Global, esta base de datos contendra

// los datos globales que no dependen de la asignatura, lo que permitirá
...
// la version 2.0 de Sosem, que tendrá uso comercial.
	...
	//global $GlobalDB; 
		...
		$ResponsePointer=mysqli_query($ConLink,$QueryStr);//$ResponsePointer=mysql_db_query($DataBasePrefix.$GlobalDB,$QueryStr,$ConLink) or die ("Parámetros enviados: Prefijo=> $DataBasePrefix; Nombre de Base de datos=> SosemDB; Usuario de Base de datos=> $DataBaseUser; Consulta=> $QueryStr; Errorde MySQL=> ".mysql_error());
// Puede ser que se provoque un error con las consultas que no retornan respuesta como el Update o Insert
		if ($Retorna) {
			$nQueryRec=mysqli_num_rows($ResponsePointer);
		}
