<!-------------------------------------------------- FOOTER -------------------------------------------->

<!--end page-content" -->
</div>


<!-- Optional JavaScript -->
<!-- jQuery first, then Bootstrap JS -->
<script src="<?php echo JS_DIR; ?>/jquery-3.3.1.slim.js"></script>
<script src="<?php echo JS_DIR; ?>/ajax.popper.1.12.9.min.js"></script>
<script src="<?php echo JS_DIR; ?>/bootstrap.min.4.3.1.js"></script>
<script src="<?php echo JS_DIR; ?>/ajax.googleapis.3.4.1.jquery.min.js"></script>
<script src="<?php echo JS_DIR; ?>/jquery.dataTables.1.10.19.min.js"></script>
<script src="<?php echo JS_DIR; ?>/dataTables.bootstrap4.1.10.19.min.js"></script>
<script src="<?php echo JS_DIR; ?>/chart.js@2.8.0.js"></script>
<script src="<?php echo JS_DIR; ?>/sweetalert2.all.min.js"></script>
<script src="https://cdn.jsdelivr.net/npm/promise-polyfill"></script>
<script src="<?php echo JS_DIR; ?>/scripts.js"></script>
<script src="<?php echo JS_DIR; ?>/cs_search_clientes_config.js"></script>
<script src="<?php echo JS_DIR; ?>/cs_load_pictures_config.js"></script>