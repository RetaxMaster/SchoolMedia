...
// Se valida que el usuario que realiza la solicitud se encuentre logged y que la session siga activa
    ...
    include_once(LIBRARY_DIR."/disponibilidad.php"); // Se incluyen las funciones de control de paises
    $enabled=(isset($_GET["enbd"])) ? $_GET["enbd"] : -1; // se recuperan los parametros de la consulta
    disp_recoveryAllList($n, $Arry, $enabled, true); // Se recuperan todos los registros de todos los paises, activos o no
