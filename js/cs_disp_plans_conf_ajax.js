...
function onPageStart() {
    ...
    setTableLabels('#tablaVerTodos', LangLabelsURL, true, './ajax_disp_plans_rcvry.php?Lang=' + globalLang + '&enbd=2&UID=' + getCookie("UID") + '&USS=' + getCookie("USS") + '', function (res) {
    ...
    $(document).on("click", ".updateData", function () {
        ...
        getDataOfThisRecord(idToUpdate, "getDispPlansData", {
            idCliente: 0,
            idplan: 1,
            country: 2,
            Provincia: 3,
    ...
    $("#idFormDetalles").on("submit", function (e) {
        ...
        if (validateInputs(inputs)) {
            ...
            if (isUpdating) {
                formData.append("mode", "updateDispPlansInfo");
                formData.append("idToUpdate", idToUpdate);
                successText = "¡Registro actualizado con éxito!";
            } else {
                formData.append("mode", "uploadDispPlansInfo");
