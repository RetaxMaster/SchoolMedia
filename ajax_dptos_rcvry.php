...
// Se valida que el usuario que realiza la solicitud se encuentre logged y que la session siga activa
    ...
    include_once(LIBRARY_DIR."/dpto_empres.php"); // Se incluyen las funciones de control de paises
 // Se recogen los parametros de la consulta
    dpto_recoveryAllList($nDptos,$DptosArry); // Se recuperan todos los registros de todos los departamentos empresariales
    JSonformatedData($nDptos,$DptosArry,$JSonDataObj); // Se hace el formato de Cadena embebido JSon para ser enviado. Pertenece a Shared
